...
The `home-routes` handler in the `libre-my-neon.routes.home` namespace
...
The `home-page` function will in turn call the `libre-my-neon.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `libre-my-neon.handler` namespace:
...
Request middleware functions are located under the `libre-my-neon.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `libre-my-neon.dev-middleware` namespace found in
